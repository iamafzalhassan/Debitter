package com.example.debitter.ui.theme

import androidx.compose.ui.unit.Dp
import androidx.compose.ui.unit.dp

object AppSpacing {
    val amountColumn: Dp = 132.dp
    val appBarHeight: Dp = 64.dp
    val borderFocus: Dp = 1.4.dp
    val cardPadding: Dp = 12.dp
    val chargeRowGap: Dp = 8.dp
    val chargeRowHeight: Dp = 96.dp
    val controlHeight: Dp = 52.dp
    val emptyStateIcon: Dp = 80.dp
    val hairline: Dp = 1.dp
    val iconButton: Dp = 18.dp
    val iconButtonSize: Dp = 40.dp
    val iconEmptyState: Dp = 32.dp
    val iconHint: Dp = 16.dp
    val iconInline: Dp = 14.dp
    val iconPlaceholder: Dp = 24.dp
    val iconSheet: Dp = 22.dp
    val iconTile: Dp = 20.dp
    val indexColumn: Dp = 28.dp
    val lg: Dp = 16.dp
    val md: Dp = 12.dp
    val previewPageMaxWidth: Dp = 480.dp
    val progressIndicator: Dp = 20.dp
    val progressStroke: Dp = 2.dp
    val radiusButton: Dp = 50.dp
    val radiusCard: Dp = 12.dp
    val radiusField: Dp = 10.dp
    val screenPadding: Dp = 16.dp
    val sm: Dp = 8.dp
    val totalsRowHeight: Dp = 34.dp
    val touchTarget: Dp = 48.dp
    val xl: Dp = 24.dp
    val xs: Dp = 4.dp
    val xxs: Dp = 2.dp

    val buttonHeight: Dp = controlHeight
    val chargeRowStride: Dp = chargeRowHeight + chargeRowGap
}
